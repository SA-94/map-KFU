...
المتصفحات وعامل الخدمة يخزّنان الملفات، فبدون تغيير رقم النسخة قد يبقى
الطلاب على النسخة القديمة. غيّر الرقم في ثلاثة أماكن (يجب أن تتطابق):

| الملف | ما تغيّره |
|---|---|
| `index.html` | كل `?v=1.1.0` في وسوم `<link>` و`<script>` |
| `js/script.js` | ثابت `ASSET_VERSION` |
| `sw.js` | ثابت `VERSION` |
